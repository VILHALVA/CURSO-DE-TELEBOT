...
function plugin.onTextMessage(msg, blocks)
	...
	if u.is_allowed('texts', msg.chat.id, msg.from) then
		if not blocks[2] then
			...
			if pin_id then
				api.sendMessage(msg.chat.id, i18n('Last message generated by `/pin` ^'), true, nil, pin_id)
			end
			return
		end
		...
		if pin_id and blocks[1] ~= "newpin" then --try to edit the old message
			...
			if not res then
				...
				api.sendMessage(msg.chat.id, i18n("Message edited. Check it here"), nil, nil, pin_id)
			end
		end
